...
public class Trueque extends ComandosEscucha {
	...
	public void ejecutar() {
		...
		try {
			casta = (Casta) Class
					.forName("dominio" + "." + juego.getCliente().getPaquetePersonaje().getCasta())
					.newInstance();
			pj = (Personaje) Class
					.forName("dominio" + "." + juego.getCliente().getPaquetePersonaje().getRaza())
		...
		if (id == paqueteComerciar.getId()) {
			...
			ArrayList<Item> items = juego.getPersonajesConectados().get(paqueteComerciar.getIdEnemigo())
					.getItems();
			boolean loop = true;
			int i = 0;
			while (juego.getCliente().getM1().getObtener().size() > 0) {
				while (loop) {
					if (items.get(i).getNombre()
							.equals(juego.getCliente().getM1().getObtener().get(0))) {
			...
			pj.trueque(juego.getCliente().getPaquetePersonaje().getItems(), paqueteComerciar.getItemsADar(),
					juego.getCliente().getM1().getDar());
		} else {
